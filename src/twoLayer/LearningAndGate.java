...
public class LearningAndGate {
    ...
    public static void main(String[] args) throws IOException {
        ...
        LerningLogicGates and = new LerningLogicGates(
                numInput, correctResponse, answerWeight, responseFunction, random);
        List<Point2D.Double> plist = and.learning(max, c);
        try ( BufferedWriter out = LerningLogicGates.openWriter("andGate.txt")) {
        ...
        for (int i = -1; i < 2; i += 2) {
            for (int j = -1; j < 2; j += 2) {
                ...
                double r = and.checkResponse(input);
